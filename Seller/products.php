...
?>
    <div class="menu" style="background:#1976d2;display:flex;gap:18px;padding:0 24px 0 24px;border-radius:10px 10px 0 0;box-shadow:0 2px 8px rgba(25,118,210,0.08);margin-bottom:32px;align-items:center;">
        ...
        <a href="products.php" class="active" style="color:#fff;text-decoration:none;font-size:1.1rem;padding:18px 22px;display:inline-block;font-weight:600;background:#1251a3;border-radius:8px 8px 0 0;">Mes produits</a>
        <a href="orders.php" style="color:#fff;text-decoration:none;font-size:1.1rem;padding:18px 22px;display:inline-block;font-weight:600;transition:background 0.2s;border-radius:8px 8px 0 0;" onmouseover="this.style.background='#1251a3'" onmouseout="this.style.background='none'">Commandes</a>
        <a href="../compte/login.php" style="color:#fff;text-decoration:none;font-size:1.1rem;padding:18px 22px;display:inline-block;font-weight:600;transition:background 0.2s;border-radius:8px 8px 0 0;margin-left:auto;background:#d32f2f;" onmouseover="this.style.background='#b71c1c'" onmouseout="this.style.background='#d32f2f'">Déconnexion</a>
    </div>
    <div class="content" style="background:#f8fafc;padding:32px 24px 40px 24px;border-radius:16px;box-shadow:0 4px 18px rgba(25,118,210,0.07);max-width:1100px;margin:32px auto 0 auto;">
        ...
        <table style="width:100%;background:#fff;border-radius:12px;box-shadow:0 2px 8px rgba(0,0,0,0.07);overflow:hidden;border-collapse:separate;border-spacing:0;">
            ...
            if (isset($_POST['ajouter_produit'])) {
                ...
                if (isset($_FILES['photo']) && $_FILES['photo']['error'] === UPLOAD_ERR_OK) {
                    ...
                    if (!is_dir($uploadDir)) {
                        mkdir($uploadDir, 0777, true); // Création du dossier avec des permissions appropriées
                    }

                    // Correction du chemin pour enregistrer les fichiers avec le bon préfixe
                ...
                $sql = "INSERT INTO produit (nom, description, prix, quantite_stock, id_vendeur, photo) VALUES ('$nom', '$description', $prix, $quantite_stock, $id_vendeur, '$photoPath')";
                if ($conn->query($sql)) {
                    $_SESSION['message'] = '<div style="color:green;margin-bottom:10px;">Produit ajouté avec succès !</div>';
                    header("Location: products.php"); // Redirection après l'ajout
                    exit();
                } else {
                    echo '<div style="color:red;margin-bottom:10px;">Erreur lors de l\'ajout du produit.</div>';
                }
            }
        ...
        if (isset($_POST['ajouter_publicite'])) {
            ...
            if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
                ...
                $imagePath = 'Seller/uploads/' . $uniqueFileName;
                move_uploaded_file($imageTmpPath, $uploadDir . '/' . $uniqueFileName);
            }

            $sql = "INSERT INTO publicite (titre, description, image, id_vendeur) VALUES ('$titre', '$description', '$imagePath', $id_vendeur)";
            if ($conn->query($sql)) {
